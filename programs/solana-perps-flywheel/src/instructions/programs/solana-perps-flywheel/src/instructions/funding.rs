...
m.funding_rate_fp = premium_fp; m.last_funding_ts = now; Ok(())
}


#[derive(Accounts)]
pub struct SettleFunding<'info> { #[account(mut)] pub market: Account<'info, Market>, pub oracle: Account<'info, OraclePrice> }

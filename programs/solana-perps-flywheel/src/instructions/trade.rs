...
up.owner = ctx.accounts.user.key(); up.market = ctx.accounts.market.key();
up.is_long = is_long;
up.base_size = if is_long { base_size_units as i64 } else { -(base_size_units as i64) };
up.entry_price_fp = price_fp; up.margin_deposited = margin; up.last_funding_settled = Clock::get()?.unix_timestamp;
...
let up = &mut ctx.accounts.user_position;
let signed_base = up.base_size as i128; let entry_fp = up.entry_price_fp as i128;
let notional_entry_fp = (signed_base.abs() as i128) * entry_fp; let notional_exit_fp = (signed_base.abs() as i128) * (mark_fp as i128);
...
up.base_size = 0; up.margin_deposited = 0; Ok(())
}
...
#[account(mut)] pub market: Account<'info, Market>,
pub oracle: Account<'info, OraclePrice>,
#[account(mut, seeds=[b"pos", user.key().as_ref(), market.key().as_ref()], bump)] pub user_position: Account<'info, UserPosition>,
#[account(mut)] pub user_token: Account<'info, TokenAccount>,
#[account(mut)] pub vault_token: Account<'info, TokenAccount>,

...
pub fn set_fee_destination(ctx: Context<AdminOnly>, new_fee_dest: Pubkey) -> Result<()> { ctx.accounts.config.fee_destination = new_fee_dest; Ok(()) }
pub fn edit_max_position(ctx: Context<AdminOnlyMarket>, new_max_base: u64) -> Result<()> { ctx.accounts.market.max_position_base = new_max_base; Ok(()) }
pub fn pause(ctx: Context<AdminOnly>, paused: bool) -> Result<()> { ctx.accounts.config.paused = paused; Ok(()) }
...
#[derive(Accounts)]
pub struct AdminOnly<'info> { #[account(mut, has_one = admin)] pub config: Account<'info, Config>, pub admin: Signer<'info> }


#[derive(Accounts)]
pub struct AdminOnlyMarket<'info> { pub admin: Signer<'info>, #[account(mut)] pub market: Account<'info, Market> }
